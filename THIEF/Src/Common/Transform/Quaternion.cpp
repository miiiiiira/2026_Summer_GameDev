...
Quaternion Quaternion::Euler(double radX, double radY, double radZ)
{
    ...
    radX = Math::RadIn2PI(radX);
    radY = Math::RadIn2PI(radY);
    radZ = Math::RadIn2PI(radZ);
...
Quaternion Quaternion::LookRotation(VECTOR dir, VECTOR up)
{
    ...
    VECTOR right = Math::Normalize(VCross(up, dir));
    up = VCross(dir, right);
    auto m00 = right.x;
    auto m01 = right.y;
    auto m02 = right.z;
...
Quaternion Quaternion::FromToRotation(VECTOR fromDir, VECTOR toDir)
{
	...
	double angle = Math::AngleDeg(fromDir, toDir);
	if (angle >= 179.9196)
	{
		auto r = VCross(fromDir, Math::DIR_R);
	...
	return Quaternion::AngleAxis(Math::Deg2Rad(angle), axis);
}
